  ...
  return (
    <div style={{ display: "flex", gap: 16, height: "calc(100vh - 120px)" }}>
      ...
      <div style={{ width: 240, flexShrink: 0, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {groups.length > 0 && (
          <>
            ...
            {groups.map((g) => {
              ...
              return (
                <button key={g.id} onClick={() => { setBroadcastGroup(g.id); setActiveChannel(null); setSelectedBridge(null); bulk.clearSelection(); }} style={{ background: isAc ? g.color + "12" : "rgba(255,255,255,0.02)", border: `1px solid ${isAc ? g.color + "30" : "rgba(255,255,255,0.05)"}`, borderRadius: 8, padding: "10px 12px", cursor: "pointer", fontFamily: "inherit", textAlign: "left", display: "block", width: "100%" }}>
        ...
        {bridges.length > 0 && (
          <>
            <div style={{ marginTop: 8 }}><SectionTitle text="Bridge Channels" /></div>
            {bridges.map((b) => {
              ...
              return (
                <button key={b.id} onClick={() => { setSelectedBridge(isAc ? null : b.id); setActiveChannel(null); setBroadcastGroup(null); bulk.clearSelection(); }} style={{ background: isAc ? "rgba(56,189,248,0.08)" : "rgba(255,255,255,0.02)", border: `1px solid ${isAc ? "rgba(56,189,248,0.25)" : "rgba(255,255,255,0.05)"}`, borderRadius: 8, padding: "10px 12px", cursor: "pointer", fontFamily: "inherit", textAlign: "left", display: "block", width: "100%" }}>
                  <div style={{ fontSize: 11, color: isAc ? "#38bdf8" : "#a1a1aa", display: "flex", alignItems: "center", gap: 4 }}>
                    {fA?.name || "?"} <Link2 size={10} /> {tA?.name || "?"}
                  </div>
                  <div style={{ fontSize: 9, color: "#52525b", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}>
                    <span style={{ color: fNet?.color }}>{fNet?.name}</span>
                    <span>→</span>
                    <span style={{ color: tNet?.color }}>{tNet?.name}</span>
                    {bmCount > 0 && <span style={{ marginLeft: 6, color: "#38bdf8" }}>{bmCount} msgs</span>}
                  </div>
                </button>
              );
            })}
        ...
        {(activeChannel || broadcastGroup || isBridgeMode) ? (
              ...
              <div>
                ...
                {isBridgeMode && selBridgeFrom && selBridgeTo && (
                  <div>
                    ...
                    <div style={{ fontSize: 9, color: "#52525b", marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}>
              ...
              {isBridgeMode && selBridgeFrom && selBridgeTo && (() => {
                ...
                if (bMsgs.length === 0) return (
                  <div style={{ textAlign: "center", padding: 40, color: "#3f3f46" }}>
                    ...
                    <div style={{ fontSize: 11, marginTop: 8 }}>Send a message across the bridge.</div>
                  </div>
                );
                return bMsgs.map((m) => {
                  ...
                  return (
                    <div key={m.id} style={{ marginBottom: 20 }}>
                      <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                        <div style={{ width: 28, height: 28, borderRadius: 6, background: (sRole?.color || "#555") + "20", border: `1px solid ${sRole?.color || "#555"}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>{sRole?.icon}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 10, color: sRole?.color, marginBottom: 4 }}>{selBridgeFrom.name} <span style={{ color: "#3f3f46", fontSize: 8 }}>({selBridgeFromNet?.name})</span> <span style={{ color: "#3f3f46", fontSize: 9 }}>{new Date(m.ts).toLocaleTimeString()}</span></div>
                          <FormattedMessage content={m.content} className="chat-md" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "2px 10px 10px 10px", padding: "10px 14px", fontSize: 12, lineHeight: 1.6, color: "#d4d4d8" }} />
                        </div>
                      </div>
                      {m.status === "sending" && <div style={{ paddingLeft: 38, fontSize: 11, color: "#38bdf8" }}><span style={{ animation: "pulse 1.5s infinite" }}>●</span> {selBridgeTo.name} is thinking…</div>}
                      {m.response && (
                        <div style={{ display: "flex", gap: 10, paddingLeft: 38 }}>
                          <div style={{ width: 28, height: 28, borderRadius: 6, background: (rRole?.color || "#555") + "20", border: `1px solid ${rRole?.color || "#555"}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>{rRole?.icon}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 10, color: rRole?.color, marginBottom: 4 }}>{selBridgeTo.name} <span style={{ color: "#3f3f46", fontSize: 8 }}>({selBridgeToNet?.name})</span> <span style={{ color: m.status === "no-prompt" ? "#ef4444" : "#3f3f46", fontSize: 9 }}>{m.status === "no-prompt" ? "no prompt" : "response"}</span></div>
                            <FormattedMessage content={m.response} className="chat-md" style={{ background: m.status === "no-prompt" ? "rgba(239,68,68,0.05)" : (rRole?.color || "#555") + "08", border: `1px solid ${m.status === "no-prompt" ? "rgba(239,68,68,0.15)" : (rRole?.color || "#555") + "15"}`, borderRadius: "10px 2px 10px 10px", padding: "10px 14px", fontSize: 12, lineHeight: 1.6, color: m.status === "no-prompt" ? "#71717a" : "#d4d4d8", whiteSpace: "pre-wrap" }} />
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {bulk.count > 0 ? (
                ...
                <>
                  ...
                  {isBridgeMode && selBridgeTo && (
                    <div style={{ display: "flex", gap: 10 }}>
                      <input placeholder={`Message ${selBridgeTo.name} across bridge...`} value={bridgeMsgInput} onChange={(e) => setBridgeMsgInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && sendBridgeMessage()} disabled={bridgeSending} style={{ ...inputStyle, border: "1px solid rgba(56,189,248,0.15)", opacity: bridgeSending ? 0.5 : 1 }} />
                      <button onClick={sendBridgeMessage} disabled={bridgeSending || !bridgeMsgInput.trim()} style={{ background: bridgeSending ? "#3f3f46" : "#38bdf8", color: "#0a0a0f", border: "none", padding: "10px 18px", borderRadius: 6, cursor: bridgeSending ? "not-allowed" : "pointer", fontFamily: "inherit", fontSize: 11, fontWeight: 500, flexShrink: 0 }}>{bridgeSending ? "…" : "Send"}</button>
                    </div>
                  )}
                </>
              )}
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "#3f3f46" }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 12 }}>Select a channel, group, or bridge.</div>
            </div>
          </div>
        )}
      </div>
