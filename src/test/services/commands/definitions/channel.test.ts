...
describe('createChannelCommand', () => {
    ...
    it('throws if agent not found', async () => {
        ...
        await expect(createChannelCommand.execute(args, mockContext as any)).rejects.toThrow("Agent 'unknown' not found");
    });

    it('throws if creating channel to self', async () => {
        const args = { from: 'agent-1', to: 'agent-1' };
        await expect(createChannelCommand.execute(args, mockContext as any)).rejects.toThrow("Cannot create channel to self");
    });
